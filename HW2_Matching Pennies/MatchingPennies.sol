...
event LargeTransferRequested(address indexed from, address indexed to, uint256 value, uint256
nonce);
event LargeTransferApproved(address indexed from, address indexed to, uint256 value, uint256
nonce);
/**
...
*/
function _checkAndHandleLargeTransfer(address from, address to, uint256 value) internal returns
(bool) {
...
emit LargeTransferApproved(from, address(0), 0, nonce); // We don't expose receiver/value in the
event for privacy
}
}